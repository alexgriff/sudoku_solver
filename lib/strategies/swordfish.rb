class Strategy::Swordfish < Strategy::BaseStrategy
  ...
  #   n = 3
  #   cell.candidates.safe_each(Proc.new { |cand| cell.candidates.include?(cand) }) do |cand|
  ...
  #   if current_line.cells_with_candidates([cand]).length >= 2
  #     other_axis_ids_current_line = current_line.cells_with_candidates([cand]).map { |ln_cell| ln_cell.send("#{other_axis_line_type}_id") }
  #     other_lines_w_cand = (board.send("#{line_type}s") - [current_line]).select { |ln| ln.has_candidates?([cand]) }
  ...
  #     set_condition = Proc.new { |set| set.all? { |ln| ln.has_candidates?([cand]) } }
  #     other_lines_w_cand.permutation(n - 1).uniq { |perm| perm.sort }.safe_each(set_condition) do |set|
  #       other_axis_ids_in_set = set.map { |ln| ln.cells_with_candidates([cand]).map(&:"#{other_axis_line_type}_id") }
  ...
  #           cell_condition = Proc.new { |cl| cl.candidates.include?(cand) }
  #           other_axis.other_cells_with_candidates(intersecting_cells, [cand]).safe_each(cell_condition) do |other_axis_cell|

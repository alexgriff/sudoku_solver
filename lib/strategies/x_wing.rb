...
#     # for each line, look at the cands (weed out singles)
#     board.send("each_incomplete_#{line_type}") do |line|
...
#           next unless set.all? { |ln| ln.has_candidates?([cand]) } # accounts for state having changed - TODO do this better
...
#               other_axis.other_cells_with_candidates(intersecting_cells, [cand]).each do |other_axis_cell|

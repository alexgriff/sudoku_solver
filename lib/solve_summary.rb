class Solve::Summary
  Strategy::BaseStrategy.subclasses.each do |strategy|
    ...
    define_method(:"num_#{strategy.name}") do
      history.where(
        strategy: strategy.name,
        type: Action::UPDATE_CELL,
      ).map(&:strategy_application_id).uniq.length
    end
  end
  ...
  def summarize
    [
      ...
      used_strategy?(Strategy::NakedPair) && "Naked pairs: #{num_naked_pair}",
      used_strategy?(Strategy::NakedPair) && "  cells solved 'by sudoku' after identifying a naked pair: #{solved_by_naked_pair_cell_count}",
      used_strategy?(Strategy::LockedCandidatesPointing) && "Locked lines: #{num_locked_candidates_pointing}",
      ...
      used_strategy?(Strategy::LockedCandidatesClaiming) && "Claimed lines: #{num_locked_candidates_claiming}",
      ...
      used_strategy?(Strategy::NakedTriple) && "Naked triples: #{num_naked_triple}",
      ...
      used_strategy?(Strategy::NakedQuadruple) && "Naked quadruples: #{num_naked_quadruple}",
      ...
      used_strategy?(Strategy::HiddenPair) && "Hidden pairs: #{num_hidden_pair}",
      ...
      used_strategy?(Strategy::XWing) && "X-Wings: #{num_x_wing}",
      ...
      used_strategy?(Strategy::HiddenTriple) && "Hidden triples: #{num_hidden_triple}",
      ...
      used_strategy?(Strategy::RandomGuess) && "Random Guesses: #{num_random_guess}",
      "Passes: #{num_passes}",
      "(#{total_count})"
    ].compact.join("\n")
  end

            ...
            if naked_buddy_cells.length == n
              house.each_cell(house.cells - naked_buddy_cells) do |non_buddied_cell|
                board.state.register_change(
                  ...
                  {strategy: name, strategy_application_id: "#{house.class} #{house.id}|#{naked_buddy_cells.map(&:id)}"}
                )
              end
            end
          end
              ...
              if hidden_buddys.length == n
                hidden_buddys.each do |hidden_buddy_cell|
                  board.state.register_change(
                    ...
                    {strategy: name, strategy_application_id: "#{house.class} #{house.id}|#{hidden_buddys.map(&:id)}"}
                  )
                end
              end
            end
            ...
            matched_other_axis_ids.each do |id|
              ...
              other_axis_line.each_cell_with_candidates(
                ...
                board.state.register_change(
                  ...
                  {strategy: name, strategy_application_id: "#{line_type}-#{line_set.map(&:id)}|#{other_axis_line_type}-#{matched_other_axis_ids}|locked-#{cand}"}
                )
              end
            end
          end
